...
class Controller():
    def __init__(self,structure,screen):
        ...
        self.fail_textsurface = self.fail_font.render('You lose!', False, (255, 0, 0))
        ...
        if collpase:
            for truss in structure.trusses:
                ...
                for truss in structure.roadtrusses:
                    truss.collapse = True
    def initial_platform(self,map_num):
        if map_num == 1 or map_num== 2 or map_num == 3: ###################這裡可以改   
            structure = self.structure
    ...
    def update(self):
        ...
        #--------------------------
        self.limit_textsurface = self.limit_font.render('%d trusses remaining' %(1+self.structure.truss_limit-len(self.structure.trusses)), True, (255, 255, 255))
    ...
    def show_win(self):
        ...
        self.note_textsurface = self.note_font.render('PRESS A BUTTON AND BACK TO MENU!',True,(0,0,0))
